...
Call `getWarnings()` or run `mql-interpreter --list-warnings` to retrieve the current list programmatically. The list is
generated by `npm run update-warning-docs`.
...
BacktestRunner automatically calls `OnInit` before processing the first candle and invokes `OnDeinit` after the session ends. Timers can be scheduled with `EventSetTimer` to invoke `OnTimer` at second-based intervals; `EventKillTimer` cancels the schedule.
