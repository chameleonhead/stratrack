...
describe("iATR", () => {
  ...
  it("calculates average true range", () => {
    const res = iATR(context, "GBPUSD", 15, 3, 0);
    expect(res).toBeCloseTo(0.15, 4);
  });

  it("uses indicator cache", () => {
    const engine = context.indicatorEngine! as InMemoryIndicatorEngine;
    const key = { type: "iATR", symbol: "GBPUSD", timeframe: 15, params: { period: 3 } } as const;
    iATR(context, "GBPUSD", 15, 3, 0);
    const state = engine.peek<any>(key)!;
    expect(state.last).toBe(candles.length - 1);
    iATR(context, "GBPUSD", 15, 3, 1);
