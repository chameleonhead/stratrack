...
import { iBearsPower } from "../../src/ta/bears";
...
describe("iBearsPower", () => {
  ...
  it("calculates Bears Power", () => {
    const res = iBearsPower(context, "GBPUSD", 15, 3, 0, 0);
    expect(res).toBeCloseTo(-0.0531, 4);
  });
  ...
  it("uses indicator cache", () => {
    ...
    iBearsPower(context, "GBPUSD", 15, 3, 0, 0);
    const state = engine.peek<any>(key)!;
    expect(state.last).toBe(candles.length - 1);
    iBearsPower(context, "GBPUSD", 15, 3, 0, 1);
