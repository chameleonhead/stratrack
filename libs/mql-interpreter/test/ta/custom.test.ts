...
import { iCustom } from "../../src/ta/custom";
...
int OnCalculate(){return Bars;}`
  ...
  it("executes custom indicator", () => {
    const res = iCustom(context, "GBPUSD", 15, "Simple", 0, 0);
    expect(res).toBeCloseTo(1.25, 4);
  });

  it("uses indicator cache", () => {
    const engine = context.indicatorEngine! as InMemoryIndicatorEngine;
    const key = { type: "iCustom:Simple", symbol: "GBPUSD", timeframe: 15, params: [] } as const;
    iCustom(context, "GBPUSD", 15, "Simple", 0, 0);
    const state = engine.peek<any>(key)!;
    expect(state.last).toBe(candles.length - 1);
    iCustom(context, "GBPUSD", 15, "Simple", 0, 1);
