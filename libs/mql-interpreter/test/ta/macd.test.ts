...
describe("iMACD", () => {
  ...
  it("calculates macd components", () => {
    const macd = iMACD(context, "GBPUSD", 15, 2, 3, 2, 6, 0, 0);
    const signal = iMACD(context, "GBPUSD", 15, 2, 3, 2, 6, 1, 0);
    const hist = iMACD(context, "GBPUSD", 15, 2, 3, 2, 6, 2, 0);
    expect(macd).toBeCloseTo(0.0228106, 6);
    expect(signal).toBeCloseTo(0.0213413, 6);
    expect(hist).toBeCloseTo(0.0014693, 6);
  });

  it("shares cache across modes", () => {
    const engine = context.indicatorEngine! as InMemoryIndicatorEngine;
    const key = {
      type: "iMACD",
      symbol: "GBPUSD",
      timeframe: 15,
      params: { fast: 2, slow: 3, signal: 2, applied: 6 },
    } as const;
    iMACD(context, "GBPUSD", 15, 2, 3, 2, 6, 0, 0);
    const state = engine.peek<any>(key)!;
    expect(state.last).toBe(candles.length - 1);
    iMACD(context, "GBPUSD", 15, 2, 3, 2, 6, 1, 0);
