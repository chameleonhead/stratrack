...
import { iBullsPower } from "../../src/ta/bulls";
...
describe("iBullsPower", () => {
  ...
  it("calculates Bulls Power", () => {
    const res = iBullsPower(context, "GBPUSD", 15, 3, 0, 0);
    expect(res).toBeCloseTo(0.0969, 4);
  });
  ...
  it("uses indicator cache", () => {
    ...
    const key = {
      ...
      symbol: "GBPUSD",
      timeframe: 15,
      params: { period: 3, applied: 0 },
    } as const;
    iBullsPower(context, "GBPUSD", 15, 3, 0, 0);
    const state = engine.peek<any>(key)!;
    expect(state.last).toBe(candles.length - 1);
    iBullsPower(context, "GBPUSD", 15, 3, 0, 1);
