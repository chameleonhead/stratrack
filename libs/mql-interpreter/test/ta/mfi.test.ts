...
describe("iMFI", () => {
  ...
  it("calculates money flow index", () => {
    const res = iMFI(context, "GBPUSD", 15, 3, 0, 0);
    expect(res).toBe(100);
  });

  it("uses indicator cache", () => {
    const engine = context.indicatorEngine! as InMemoryIndicatorEngine;
    const key = { type: "iMFI", symbol: "GBPUSD", timeframe: 15, params: { period: 3 } } as const;
    iMFI(context, "GBPUSD", 15, 3, 0, 0);
    const state = engine.peek<any>(key)!;
    expect(state.last).toBe(candles.length - 1);
    iMFI(context, "GBPUSD", 15, 3, 0, 1);
