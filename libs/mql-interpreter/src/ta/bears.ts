...
export function iBearsPower(
  ...
  if (ctx.last < curIdx) {
    for (let i = ctx.last + 1; i <= curIdx; i++) {
      ...
      ctx.values[i] = candles[i].low - ctx.ema;
      ctx.last = i;
    }
  }
